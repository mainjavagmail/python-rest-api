...
atributos.add_argument('login', type=str, required=True, help="'login' can't be empty")
atributos.add_argument('senha', type=str, required=True, help="'senha' can't be empty")
...
class UsuarioCadastro(Resource):
    def post(self):
        ...
        if UsuarioModel.find_usuario_by_login(dados['login']):
            return {'message': 'login {} already exists!'.format(self.login)}, 400
        ...
        try:
            ...
            return {'message': 'Impossible to save user {}'.format(usuario.login)}, 400
